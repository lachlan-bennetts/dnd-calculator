...
class CharacterController {
	...
	private initializeRoutes() {
		...
		this.router.get('/characters/character/levelUp', this.handleGetLevelUpCharacter)
	...
	private handleGetLevelUpCharacter = async (req: Request, res: Response, next: NextFunction) => {
		try {
			const {error, value} = getReqCharacterInfo(req.headers);
			if(error) {
				this.logger.error('Error', error);
			...
			const result = await this.characterService.levelUpCharacterInfo(characterId, userId)
			res.status(200).json(result);
		} catch (err: any) {
			this.logger.error(`Error has occurred at handleGetLevelUpCharacter  and error ${err}`)
			if (err instanceof CustomError) {
				next(err)
			}
			const error = new CustomError(err.message, 500)
